...
export default function TodayScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (galleryStatus !== "granted") {
        Alert.alert(
          "Brak uprawnień",
          "Aby dodawać zdjęcia potrzebujemy dostępu do galerii",
        );
      }
      if (camStatus !== "granted") {
        Alert.alert(
          "Brak uprawnień",
          "Potrzebujemy dostępu do kamery, aby robić zdjęcia",
        );
      }
    })();
  }, []);
  ...
  const renderItem = ({ item, section }: { item: Habit; section: Section }) => (
    <HabitItem
      habit={item}
      ...
      {sections.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            Dodaj pierwszy nawyk w zakładce "Nawyki"
          </Text>
        </View>
      ) : (
        <SectionList
